...
class StreamDecoder {
public:
  StreamDecoder(int id, const std::string &name, const std::string &url,
...
public:
  ...
  GstElement *demux_ = nullptr;  // For file sources (qtdemux)
  static void onDemuxPadAdded(GstElement *element, GstPad *pad, gpointer data);
  ...
  // Helper to copy frame to CPU buffer
  void pushToCpuBuffer(GstBuffer* buffer, int width, int height, uint64_t frame_id);
